...
{
    // This project's source has one synthetic stream. Canon USB transport is
    // owned by the Rust worker; it never wraps Canon's webcam driver.
    ...
    {
        if (m_spSimpleMediaSrc) return m_spSimpleMediaSrc->Shutdown();
        return S_OK;
    }
