    ...
    _In_ ULONG rgbMask )
{
    ...
    for (unsigned int r = 0; r < height; r++)
    {
        ...
        {
            BYTE red = (BYTE)(c * 255 / width);
            BYTE green = (BYTE)(r * 255 / height);
            BYTE blue = (c >= (unsigned)offset && c < (unsigned)offset + 20) ? 255 : 32;
            // Asymmetric marks make rotation and mirroring errors visible.
            *p = (r < 24 || c < 24) ? 0x00ffffff : ((uint32_t)red << 16 | (uint32_t)green << 8 | blue);
            p++;
        }
    }
