...
namespace winrt::WindowsSample::implementation
{
        ...
        )
    {
        ...
        // Portrait uses actual portrait dimensions, not rotation metadata.
        ...
        )
    {
        ...
        // Bound this prototype's production rate to the advertised 30 fps.
        const LONGLONG now = MFGetSystemTime();
        if (m_nextFrameTime > now)
        {
            Sleep(static_cast<DWORD>((m_nextFrameTime - now + 9999) / 10000));
        }
        m_nextFrameTime = MFGetSystemTime() + 333333;
        ...
        const HRESULT frameResult = m_spFrameGenerator->CreateFrame(pbuf, bufferLength, pitch, m_rgbMask);
